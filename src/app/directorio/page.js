      ...
      <div className='grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8'>
        {
          DataDirectory.directory.map((anime) => (
            <article key={anime.id} className="w-80 h-auto text-start overflow-hidden rounded-xl relative transform 
            hover:-translate-y-2 transition ease-in-out duration-700 shadow-lg 
            hover:shadow-2xl hover:h-auto bg-gradient-to-t from-black via-gray-900 to-transparent">
              <Link href={`/info/${anime.title}`} class="absolute inset-0 z-10 transition duration-300 ease-in-out bg-gradient-to-t from-black via-gray-900 to-transparent"></Link>
